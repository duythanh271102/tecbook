...
function techbook_standards_tag_page() {
    global $wpdb;
    ...
    $results = $wpdb->get_results(
        $wpdb->prepare("SELECT id, standardTitle FROM $table_name WHERE `$column` = %d", 1),
        ARRAY_A
    );
    ...
    foreach ($valid_filters as $valid_filter) {
        ...
        echo '<a href="?page=techbook_standards_tag_page&filter=' . esc_attr($valid_filter) . '" class="' . esc_attr($button_class) . '">' . esc_html($button_label) . '</a> ';
    }
    echo '</div>';
    ...
    if (!empty($results)) {
        ...
        foreach ($results as $row) {
            ...
            echo '<td>' . esc_html($row['standardTitle']) . '</td>';
            echo '<td>';
